const Game = (function () {
  ...
  function createBomb (color = Math.random() > 0.5 ? 'red' : 'black')  {
    ...
    const fast = Math.random() < 0.05 ? 3 : 1
    const width = Math.random() < 0.10 ? 160 : 80
    ...
    return {
      ...
      savedX: coords.x,
      savedY: coords.y,
      offsetY: offsetY * fast,
      offsetX: offsetX * direction * fast,
      color: color,
      isDragging: false,
      canMove: true,
      elapsedFrames: 0,
  ...
  function drawPlayerScore () {
    ...
    scoreCtx.font = '64px serif'
    scoreCtx.fillStyle = 'white'
    scoreCtx.fillText('Score: ' + playerScore, 0, 50)
  ...
  document.getElementById('resetButton').addEventListener('touchstart', resetGame)
  document.getElementById('resetButton').addEventListener('click', resetGame)
  document.getElementById('fullscreenButton').addEventListener('touchstart', fullscreen)
